'use client'

import {Card, CardContent, CardHeader} from "../ui/card";
import {ChevronRight, Lock, Trash2} from "lucide-react";
import {useIsMobile} from "@/hooks/shared/use-mobile";
import {Button, buttonVariants} from "../ui/button";
import {
    Dialog,
    DialogContent,
    DialogHeader,
    DialogTitle,
    DialogTrigger,
} from "@/components/ui/dialog"
import ChangeEmailForm from "../forms/ChangeEmailForm";
import {useHasMounted} from "@/hooks/shared/useHasMounted";
import {cn} from "@/lib/utils";


export default function Account() {
    const isMobile = useIsMobile();
    const {hasMounted} = useHasMounted()

    if (!hasMounted) {
        return <div className="w-full animate-pulse">
            <div className="h-48 w-full bg-muted/20 rounded-xl"/>
        </div>
    }

    return <div className="w-full lg:col-span-6">
        {isMobile && (
            <div className="flex gap-2 items-center mb-4">
                <Lock className="stroke-chart-2"/>
                <h3 className="font-bold">Security & Plan</h3>
            </div>
        )}

        <Card className="bg-transparent lg:bg-card ring-0 lg:ring-1 -mt-4 lg:mt-0 px-1 lg:px-6 lg:h-full">
            {
                !isMobile && (
                    <CardHeader className="p-0">
                        <div className="flex justify-between items-center mb-4">
                            <div className="text-left gap-1 items-start  flex flex-col">
                                <h3 className="font-bold text-lg text-foreground">Account and Security</h3>
                                <p>Sensitive account actions</p>
                            </div>
                            <Lock className="stroke-chart-2"/>
                        </div>
                    </CardHeader>
                )
            }
            <CardContent className="flex flex-col gap-2 px-0 pb-3 lg:gap-3">
                <div>

                    <Dialog>
                        <DialogTrigger className="w-full">
                            <span
                                className={cn(buttonVariants({variant: 'outline'}), 'w-full lg:border lg:border-border py-7 flex justify-between items-center px-5')}>
                                <p>Change Email</p>
                                <ChevronRight/>
                            </span>
                        </DialogTrigger>
                        <DialogContent>
                            <DialogHeader>
                                <DialogTitle className={'text-primary'}>Change Email Address</DialogTitle>
                                <ChangeEmailForm/>
                            </DialogHeader>
                        </DialogContent>
                    </Dialog>
                </div>
                <div>
                    <Button className="w-full lg:border lg:border-border py-7  flex justify-between items-center px-5"
                            variant={'outline'}>
                        <p className="text-destructive">Logout</p>
                        <ChevronRight className="stroke-destructive"/>
                    </Button>
                </div>
                <div>
                    <Button className="w-full lg:border lg:border-border py-7 flex justify-between items-center px-5"
                            variant={'destructive'}>
                        <p>Delete Account</p>
                        <Trash2/>
                    </Button>
                </div>
            </CardContent>
        </Card>

    </div>
}


